...
char *printf_md5(unsigned char *in) //MD5 checksum
{
...
long get_file_size(char *file_name)
{
	...
	{
		printf("####L(%d) file:%s not exist!", __LINE__, file_name); //__LINE__ 位置信息
		return -1;
	}
...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		if (send(new_server_socket, str, sizeof(str), 0) < 0)
		{
		...
		else
		{
			...
			while ((file_block_length = fread(buffer, sizeof(char), BUFFER_SIZE, fp)) > 0)
			{
				...
				file_size -= file_block_length;

				printf("SendRest:%ld\n", file_size);

				printf("Sendpack_total : %s\n", str);
				...
				bzero(buffer, sizeof(buffer));
				//	usleep(500); //要休眠一下 否则第二次发过去的数据可能来不及接收到
			}

			fclose(fp);
			printf(KGRN "File:\t%s Transfer Finished!", file_name);
			printf("\n" RESET);
		...
		fread(filetotal, sizeof(char), file_size, fp);
		unsigned char digest[MD5_DIGEST_LENGTH];
		MD5_CTX context;
		MD5_Init(&context);
		MD5_Update(&context, filetotal, strlen(filetotal));
		MD5_Final(digest, &context);
		message = printf_md5(digest);
