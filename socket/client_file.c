...
int main(int argc, char *argv[])
{
    ...
    recv(client_socket, receiveMessage, BUFFER_SIZE, 0);
    printf("Packlength : %s\n", receiveMessage);
    sscanf(receiveMessage, "%d", &Packlength);
    printf("integer Packlength : %d\n", Packlength);
    ...
    //socket的建立
    int message_sockfd = 0;
    message_sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if (message_sockfd == -1)
    {
    ...
    struct sockaddr_in message_info; //對方的資訊
    bzero(&message_info, sizeof(message_info));
    message_info.sin_family = PF_INET;

    //localhost test
    message_info.sin_addr.s_addr = inet_addr("192.168.1.206");
    message_info.sin_port = htons(8080);

    int err = connect(message_sockfd, (struct sockaddr *)&message_info, sizeof(message_info));
    if (err == -1)
    {
    ...
    recv(message_sockfd, receiveMessage2, sizeof(receiveMessage2), 0);
    printf("Packlength : %s\n", receiveMessage2);
